...
export default function FirasaTab() {
  ...
  const run = async () => {
    ...
    try {
      let content;
      if (inputMode === 'image') {
        if (!sharedImage) { setError('اختر صورة أولاً'); setLoading(false); return; }
        content = [
        ...
        if (!desc.trim()) { setError('اكتب وصفاً أولاً'); setLoading(false); return; }
  ...
  return (
    <div className="fade-in flex flex-col gap-4">
      ...
      <div className="inline-flex w-fit gap-1 rounded-xl border p-1 surface">
        {[['image', 'من صورة'], ['desc', 'من وصف']].map(([id, lb]) => (
      ...
      {inputMode === 'image' ? <SharedImage compact /> : (
        <textarea value={desc} onChange={(e) => setDesc(e.target.value)} rows={5} placeholder="صف الملامح: شكل الوجه، العينان، الجبهة، الفم…"
      ...
      <div className="rounded-lg border border-amber-300/40 bg-amber-50 p-3 text-xs text-amber-700 dark:bg-amber-500/10 dark:text-amber-400">
        تنبيه: الفراسة تُصنَّف علمياً كعِلم زائف؛ القراءة للاستكشاف والترفيه فقط.
      </div>
      <Button onClick={run} disabled={loading}>
        {loading ? <Loader2 size={18} className="animate-spin" /> : <Sparkles size={18} />}
        {loading ? 'جارٍ القراءة…' : 'اقرأ الفراسة'}
      </Button>
      {error && <Card className="text-sm text-red-500">{error}</Card>}
      <ResultView title={`${cur.label}${subject ? ' — ' + subject : ''}`} text={result} />
    </div>
  );
}
