// تصدير تقارير احترافية بمعيار عصري إلى PDF (طباعة) أو HTML — بلا اعتماديات.
const BASE = `
...
h3{color:#0e7490;font-size:17px;background:#0e74900f;border-right:4px solid #0e7490;border-radius:8px;padding:7px 12px;margin:16px 0 6px}
...
.toc ol{margin:0;padding:0 20px 0 0}
...
function radarSVG(vals) {
  const labs = ['الانفتاح','الضمير الحي','الانبساط','المقبولية','الاتزان'];
  ...
  return `<div style="text-align:center;margin:8px 0 16px"><div style="font-size:13px;font-weight:700;color:#0e7490;margin-bottom:4px">مخطط سمات OCEAN</div><svg viewBox="0 0 300 260" width="330" style="max-width:100%">${rings}${axes}<polygon points="${poly}" fill="#0e7490" fill-opacity="0.25" stroke="#0e7490" stroke-width="2"/>${dots}${texts}</svg></div>`;
}
function barsHTML(text) {
  let html='';
  const oc = (text || '').match(/OCEAN[^\]]*O\s*=\s*(\d+)[^\d]+C\s*=\s*(\d+)[^\d]+E\s*=\s*(\d+)[^\d]+A\s*=\s*(\d+)[^\d]+N\s*=\s*(\d+)/i);
  if (oc) html += radarSVG(oc.slice(1,6).map(n=>parseInt(n,10)));
  const mt = (text || '').match(/\[\[?\s*METRICS([^\]]*)\]?\]/i);
  if (mt) {
    const rows=[];
    mt[1].split('|').forEach((p)=>{ const m=p.match(/(.+?)\s*=\s*(\d{1,3})/); if(m) rows.push([m[1].trim(), m[2]]); });
...
function md2html(text) {
  return (text || '').replace(/\[\[?\s*OCEAN[^\]]*\]?\]/gi, '').split('\n').map((l) => {
    const t = l.trim();
    if (!t) return '';
    if (/^#{1,3}\s/.test(t)) return `<h3>${t.replace(/^#{1,3}\s/, '')}</h3>`;
    if (/^[-*•]\s/.test(t)) return `<li>${inline(t.replace(/^[-*•]\s/, ''))}</li>`;
    return `<p>${inline(t.replace(/^\d+[.)]\s/, (m) => m))}</p>`;
  }).join('\n').replace(/(<li>[\s\S]*?<\/li>\n?)+/g, (m) => `<ul>${m}</ul>`);
}
function shell(title, css, inner) {
  return `<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><title>${title}</title><style>${BASE}${css}</style></head><body>${inner}<div class="wrap"><div class="foot">المحلّل الذكي للشخصية — ${new Date().toLocaleString('ar')}</div></div></body></html>`;
}
...
export function exportReport(title, text, mode = 'pdf') {
  const inner = `<div class="hero" style="padding:26px"><h1 style="font-size:24px">${title}</h1><div class="meta">${new Date().toLocaleDateString('ar')}</div></div><div class="wrap">${barsHTML(text)}${md2html(text)}</div>`;
  output(shell(title, FULL, inner), mode, title);
}

export function exportFullReport({ subject, sections, imageDataUrl, profile }, mode = 'pdf') {
  const named = subject && subject !== 'عام';
  const name = named ? subject : 'تقرير شامل';
  const hero = `<div class="hero">
    <h1>التقرير الشامل${named ? ' — ' + subject : ''}</h1>
    <div class="meta"><span>${named ? subject : 'غير محدّد'}</span><span>${new Date().toLocaleDateString('ar')}</span><span>${sections.length} أقسام</span></div>
    ${imageDataUrl ? `<img src="${imageDataUrl}" alt="">` : ''}
  </div>`;
  const LAB={name:'الاسم',age:'العمر',gender:'الجنس',height:'الطول',weight:'الوزن',health:'الحالة الصحية',notes:'ملاحظات'};
  const prows = profile ? Object.keys(LAB).filter(k=>String(profile[k]||'').trim()).map(k=>{let v=String(profile[k]).trim(); if(k==='height')v+=' سم'; if(k==='weight')v+=' كجم'; if(k==='age')v+=' سنة'; return [LAB[k],v];}) : [];
  const profileHTML = prows.length ? `<div class="toc"><b>المعلومات الشخصية</b><div style="display:flex;flex-wrap:wrap;gap:10px 24px">${prows.map(([k,v])=>`<div style="font-size:14px"><span style="color:#64748b">${k}: </span><b>${v}</b></div>`).join('')}</div></div>` : '';
  const toc = `<div class="toc"><b>محتويات التقرير</b><ol>${sections.map((s) => `<li>${s.type}</li>`).join('')}</ol></div>`;
  const body = sections.map((s, i) =>
    `<div class="section"><div class="head"><span class="num">${i + 1}</span><b>${s.type}</b></div><div class="body">${barsHTML(s.full)}${md2html(s.full)}</div></div>`
  ).join('\n');
  output(shell(name, FULL, hero + `<div class="wrap">${profileHTML}${toc}${body}</div>`), mode, name);
}
